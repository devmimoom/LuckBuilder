...
class AnalysisTask {
  ...
  final List<Map<String, String>> solutions; // 解法列表 [{title: "標題", content: "內容"}]
...
class AnalysisQueue extends _$AnalysisQueue {
  ...
  Future<void> startAnalysis({
    ...
    _processQueue(imagePath, rects, displaySize, erasePaths).catchError((error) {
      debugPrint("❌ _processQueue 發生錯誤: $error");
    });
  }

  Future<void> _processQueue(String imagePath, List<Rect> rects, Size displaySize, List<Path> erasePaths) async {
    ...
    debugPrint("圖片處理完成 (含 EXIF 校正): ${originalImage.width}x${originalImage.height}");
    ...
    for (int i = 0; i < rects.length; i++) {
      debugPrint("=== 處理任務 $i ===");
      _updateTask(i, status: AnalysisStatus.processing, title: '正在裁切與辨識題目 ${i + 1}...');
      ...
      try {
        ...
        debugPrint("實際可見區域: ${actualVisibleWidth}x$actualVisibleHeight, 偏移: ($offsetX, $offsetY)");
        ...
        final int left = ((rect.left - offsetX) * scale).toInt().clamp(0, originalImage.width - 1);
        final int top = ((rect.top - offsetY) * scale).toInt().clamp(0, originalImage.height - 1);
        ...
        // 儲存裁切後的圖片
        final cropFileName = 'crop_${DateTime.now().millisecondsSinceEpoch}_$i.jpg';
        ...
        _updateTask(i, progress: 0.3, cropPath: cropPath, title: '正在 Gemini OCR 辨識題目...');
        ...
        if (geminiResult != null) {
          ...
          final rawConcepts = geminiResult['key_concepts'] as List<dynamic>? ?? [];
  ...
  Future<void> _processSingleTask(
    ...
    try {
      ...
      _updateTask(taskIndex, status: AnalysisStatus.processing, title: '正在裁切題目...');
      ...
      // 儲存裁切後的圖片
      final cropFileName = 'crop_${DateTime.now().millisecondsSinceEpoch}_$taskIndex.jpg';
      final cropPath = p.join(tempDir.path, cropFileName);
      await File(cropPath).writeAsBytes(img.encodeJpg(croppedImage));

      _updateTask(taskIndex, progress: 0.3, cropPath: cropPath, title: '正在 Gemini OCR 辨識題目...');

      // 使用 Gemini 進行 OCR
      final latex = await GeminiService().recognizeImage(File(cropPath));

      if (latex == null || latex.isEmpty) {
        _updateTask(taskIndex, status: AnalysisStatus.failed, title: 'OCR 辨識失敗');
        return;
      }
  ...
  img.Image _applyEraseMask(img.Image originalImage, Size displaySize, List<Path> erasePaths) {
    // 計算座標轉換參數（與裁切邏輯相同）
    final double imgAspect = originalImage.width / originalImage.height;
    final double viewAspect = displaySize.width / displaySize.height;
    ...
    // 創建一個新的圖片副本（不修改原圖）
    final maskedImage = img.copyResize(originalImage, width: originalImage.width, height: originalImage.height);
      ...
      for (final metric in pathMetrics) {
        ...
        for (double t = 0.0; t <= 1.0; t += 0.01) {
          ...
          if (tangent != null) {
            ...
            // 座標轉換：先扣除黑邊偏移量，再縮放到圖片像素
            final pixelX = ((screenPoint.dx - offsetX) * scale).toInt().clamp(0, originalImage.width - 1);
            final pixelY = ((screenPoint.dy - offsetY) * scale).toInt().clamp(0, originalImage.height - 1);
            pathPoints.add(img.Point(pixelX, pixelY));
          }
        }

        // 使用筆刷寬度（30.0 屏幕單位）轉換為像素寬度
        final brushWidth = (30.0 * scale).toInt().clamp(5, 100); // 最小 5 像素，最大 100 像素
        ...
        for (final point in pathPoints) {
          ...
          for (int dy = -radius; dy <= radius; dy++) {
            for (int dx = -radius; dx <= radius; dx++) {
              final px = (x + dx).toInt();
              final py = (y + dy).toInt();
              if (px >= 0 && px < maskedImage.width && py >= 0 && py < maskedImage.height) {
