...
3. **數學符號**（必須完整保留）：
   ...
   - 指數符號：²、³、ⁿ 等
   - 運算符號：+、−、×、÷、=、<、>、≥、≤、≠ 等
4. **數學公式**（使用 LaTeX 格式，行內公式用 \\( ... \\)，區塊公式用 \\[ ... \\]）

**特別注意**：
- 如果題目包含 √（根號）、²（平方）、³（立方）等符號，**必須**在輸出中完整保留這些符號
- 不要將 √ 寫成「根號」或「sqrt」，要保留原始符號 √
- 不要將 ² 寫成「平方」或「^2」，要保留原始符號 ²
...
8. **輸出前自我檢查（強制執行）**: 在生成最終 JSON 輸出之前，你 **必須** 對所有包含數學公式的欄位進行以下檢查：\n   - 檢查一：所有 \\left 是否有對應的 \\right，且都在 \\( \\) 或 \\[ \\] 內。\n   - 檢查二：所有 \\frac、\\sqrt 等指令是否都在 \\( \\) 或 \\[ \\] 內。\n   - 檢查三：所有括號 {}、()、[] 是否完整閉合。\n   如果發現任何違規，請在輸出前自行修正。
